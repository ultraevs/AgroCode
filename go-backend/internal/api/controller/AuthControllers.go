...
func UserCreate(context *gin.Context) {
	...
	var maxID int
	err = database.Db.QueryRow("SELECT COALESCE(MAX(id), 0) FROM cows_users").Scan(&maxID)
	if err != nil {
	...
	_, err = database.Db.Exec("INSERT INTO cows_users (email, password, name) VALUES ($1, $2, $3)", body.Email, string(hashPass), uniqueName)
	if err != nil {
...
func Login(context *gin.Context) {
	...
	err := database.Db.QueryRow("SELECT email, password FROM cows_users WHERE email = $1", body.Email).Scan(&form.Email, &form.Password)
	if err == nil {
